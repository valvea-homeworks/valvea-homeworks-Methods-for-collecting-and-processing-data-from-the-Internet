...
from vacancy_db import Vacancy
...
def seach_hh(query):
    ...
    while True:
        ...
        temp = list(map(lambda x: {'_id':md5(x.a.text.encode()+x.a['href'].encode()).hexdigest(),
                                   'Зарплата': x.contents[1].text.replace('\xa0', ''), 'Ccылка': x.a['href'],
...
def seach_superjob(query):
    ...
    while True:
        ...
        temp = list(map(lambda x: {'_id':md5(x.a.text.encode()+x.a['href'].encode()).hexdigest(),
                                   'Зарплата': x.contents[1].text.replace('\xa0', ''),
                                   'Ccылка': url[:23] + x.a['href'],
                                   'Наименование вакансии': x.a.text,
        ...
        if not page.find('a', attrs={'class': 'f-test-button-dalshe'}):
            break
    return {'superjob.ru': base}


def get_vacancys():
    with ThreadPool(2) as th:
        return th.map(lambda x:x(query),[seach_hh,seach_superjob])



def input_update_db(dict_obj):
    vacancy_db.update_site_vacancy(tuple(dict_obj.keys())[0],
                                   dict_obj[tuple(dict_obj.keys())[0]])

vacancy_db = Vacancy()

if __name__ == '__main__':

    results = get_vacancys()
    with Pool(2) as p:
        p.map(input_update_db,results)

    pprint(vacancy_db.seach_vacancy_on_salary('hh.ru', 50000))

    pprint(vacancy_db.seach_vacancy_on_salary('superjob.ru', 20000))
